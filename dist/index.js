...
var _CFS = class {
  ...
  static readFileConfig(filePath, key2) {
    if (!filePath.startsWith(process.cwd()))
      import_path.default.join(process.cwd(), filePath);
  ...
  static encryptTransport(str, key2) {
    if (typeof str === "string") {
      const cipher = import_crypto.default.createCipheriv(constants_default.algorithm, import_crypto.default.scryptSync(key2, "salt", 24), Buffer.alloc(16));
    ...
    Object.entries(str).forEach(([oKey, val]) => {
      if (typeof val === "undefined")
        return;
      let cipher = import_crypto.default.createCipheriv(constants_default.algorithm, import_crypto.default.scryptSync(key2, "salt", 24), Buffer.alloc(16));
      let newKey = cipher.update(oKey, "utf-8", "hex") + cipher.final("hex");
      cipher = import_crypto.default.createCipheriv(constants_default.algorithm, import_crypto.default.scryptSync(key2, "salt", 24), Buffer.alloc(16));
  ...
  static decryptTransport(str, key2) {
    if (typeof str === "object") {
      ...
      Object.entries(str).forEach(([oKey, string]) => {
        let decipher2 = import_crypto.default.createDecipheriv(constants_default.algorithm, import_crypto.default.scryptSync(key2, "salt", 24), Buffer.alloc(16));
        const newOKey = decipher2.update(oKey, "hex", "utf-8") + decipher2.final("utf-8");
        decipher2 = import_crypto.default.createDecipheriv(constants_default.algorithm, import_crypto.default.scryptSync(key2, "salt", 24), Buffer.alloc(16));
        const newString = decipher2.update(string, "hex", "utf-8") + decipher2.final("utf-8");
        builder[newOKey] = newString;
      });
      return builder;
    }
    let decipher = import_crypto.default.createDecipheriv(constants_default.algorithm, import_crypto.default.scryptSync(key2, "salt", 24), Buffer.alloc(16));
...
var _ClownCryption = class {
  ...
  encrypt(str, key2 = this.key, charset = this._charset) {
    const cipher = import_crypto2.default.createCipheriv(this._encryptionAlgorithm, import_crypto2.default.scryptSync(key2, this.salt, 24), Buffer.alloc(16, this.iv));
    try {
  ...
  decrypt(str, key2 = this.key, charset = this._charset) {
    const decipher = import_crypto2.default.createDecipheriv(this._encryptionAlgorithm, import_crypto2.default.scryptSync(key2, this.salt, 24), Buffer.alloc(16, this.iv));
    try {
