...
var _ClownCryption = class {
  ...
  encrypt(str, key = this.key, charset = this._charset) {
    const cipher = Crypto.createCipheriv(this._encryptionAlgorithm, Crypto.scryptSync(key, this.salt, 24), Buffer.alloc(16, this.iv));
    try {
...
var _CFS = class {
  ...
  static readStringFile(filePath, key, options) {
    ...
    if (typeof filePath === "string" && (options == null ? void 0 : options.importByFileContent) === false) {
      ...
      if (path.extname(filePath.toString()) === ".js")
  ...
  static readFileConfig(filePath, key) {
    filePath = this._serializePath(filePath).path;
    if (path.extname(filePath).endsWith("js"))
      return this._importFromJS(filePath);
  ...
  static encryptTransport(str, key) {
    if (typeof str === "string") {
      const cipher = Crypto2.createCipheriv(constants_default.algorithm, Crypto2.scryptSync(key, "salt", 24), Buffer.alloc(16));
    ...
    Object.entries(str).forEach(([oKey, val]) => {
      if (typeof val === "undefined")
        return;
      let cipher = Crypto2.createCipheriv(constants_default.algorithm, Crypto2.scryptSync(key, "salt", 24), Buffer.alloc(16));
      let newKey = cipher.update(oKey, "utf-8", "hex") + cipher.final("hex");
      cipher = Crypto2.createCipheriv(constants_default.algorithm, Crypto2.scryptSync(key, "salt", 24), Buffer.alloc(16));
  ...
  static decryptTransport(str, key) {
    if (typeof str === "object") {
      ...
      Object.entries(str).forEach(([oKey, string]) => {
        let decipher2 = Crypto2.createDecipheriv(constants_default.algorithm, Crypto2.scryptSync(key, "salt", 24), Buffer.alloc(16));
        const newOKey = decipher2.update(oKey, "hex", "utf-8") + decipher2.final("utf-8");
        decipher2 = Crypto2.createDecipheriv(constants_default.algorithm, Crypto2.scryptSync(key, "salt", 24), Buffer.alloc(16));
        const newString = decipher2.update(string, "hex", "utf-8") + decipher2.final("utf-8");
        builder[newOKey] = newString;
      });
      return builder;
    }
    let decipher = Crypto2.createDecipheriv(constants_default.algorithm, Crypto2.scryptSync(key, "salt", 24), Buffer.alloc(16));
  ...
  static _importFromJS(fPath) {
    return __require(fPath.toString());
  }
