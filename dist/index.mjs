...
var _CFS = class {
  ...
  static encryptTransport(str, key) {
    if (typeof str === "string") {
      const cipher = Crypto.createCipheriv(constants_default.algorithm, Crypto.scryptSync(key, "salt", 24), Buffer.alloc(16));
    ...
    Object.entries(str).forEach(([oKey, val]) => {
      if (typeof val === "undefined")
        return;
      let cipher = Crypto.createCipheriv(constants_default.algorithm, Crypto.scryptSync(key, "salt", 24), Buffer.alloc(16));
      let newKey = cipher.update(oKey, "utf-8", "hex") + cipher.final("hex");
      cipher = Crypto.createCipheriv(constants_default.algorithm, Crypto.scryptSync(key, "salt", 24), Buffer.alloc(16));
  ...
  static decryptTransport(str, key) {
    if (typeof str === "object") {
      ...
      Object.entries(str).forEach(([oKey, string]) => {
        let decipher2 = Crypto.createDecipheriv(constants_default.algorithm, Crypto.scryptSync(key, "salt", 24), Buffer.alloc(16));
        const newOKey = decipher2.update(oKey, "hex", "utf-8") + decipher2.final("utf-8");
        decipher2 = Crypto.createDecipheriv(constants_default.algorithm, Crypto.scryptSync(key, "salt", 24), Buffer.alloc(16));
        const newString = decipher2.update(string, "hex", "utf-8") + decipher2.final("utf-8");
        builder[newOKey] = newString;
      });
      return builder;
    }
    let decipher = Crypto.createDecipheriv(constants_default.algorithm, Crypto.scryptSync(key, "salt", 24), Buffer.alloc(16));
...
var _ClownCryption = class {
  ...
  encrypt({
    ...
    const cipher = Crypto2.createCipheriv(algorithm, Crypto2.scryptSync(key, salt, 24), Buffer.alloc(16, iv));
    try {
  ...
  decrypt({
    ...
    const decipher = Crypto2.createDecipheriv(algorithm, Crypto2.scryptSync(key, salt, 24), Buffer.alloc(16, iv));
    try {
