...
abstract class PublicCharset extends BaseCharset {
  ...
  constructor(
    name: string,
    charset: IEfficientBinaryCharset,
  ...
  constructor(
    name: string,
    charset: IEfficientBinaryCharset | IBinaryCharset | ILiteralCharset,
    aliases: string | string[] = []
  ) {
    ...
    this.aliases = Array.isArray(aliases)
      ? aliases.map((val) => val.toLowerCase().trim())
      : [aliases.toLowerCase().trim()];
  }
}
...
class BinaryCharset extends PublicCharset {
  constructor(
    name: string,
    chars: { 0: string; 1: string } | { "0": string; "1": string },
    aliases?: string | string[]
  ) {
    super(
      name,
